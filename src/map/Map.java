...
//				if(!way.contains("highway")) continue;
				...
				if(mWayId.find()) wayId = mWayId.group(1);
				if(mWayType.find()) wayType = mWayType.group(1);
			    if(mOneWay.find() && mOneWay.group(1).equals("yes")) oneWay = true;
...
//			    if(Speed.get(wayType) == -1) continue;
	...
	private void setOutAndInEdges(){
		...
		for(Edge edge: this.edgeSet){
			...
			if(this.outEdges.containsKey(nodeStart)) this.outEdges.get(nodeStart).add(edge);
			else{
				ArrayList<Edge> edgeList = new ArrayList<Edge>();
				edgeList.add(edge);
				this.outEdges.put(nodeStart, edgeList);
			}
			if(this.inEdges.containsKey(nodeEnd)) this.inEdges.get(nodeEnd).add(edge);
			else{
				ArrayList<Edge> edgeList = new ArrayList<Edge>();
				edgeList.add(edge);
