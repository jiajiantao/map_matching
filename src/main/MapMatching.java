...
//				"M2163_1", "M2173_1"
		...
		for(String route: routes) {	
			...
			for(int i = 1; i <= n; i += 1) {
				...
				//String candfile = String.format("./data/bus_point_candidate/bus_candidateMatched_R%s_%s_%s_%s.OSM", route, i, pointGap, bias);
	...
	public static void st_matching(Map map, String route, int pointGap, CandidatePreparation cp, int bias, int i) throws IOException {	
		...
		//**************** result matching **************************
		String candidatesMatchedOsm = String.format("./data/bus_point_candidate/bus_candidateMatched_R%s_%s_%s_%s.OSM", route, i, pointGap, bias);
	...
	public static CandidatePreparation getCandidatePreparation(Map map, String route, int pointGap, int bias, int i) throws IOException {
		String gpsFile = String.format("./data/bus_gps/R%s/R%s_%s.txt", route, route, i);
		String candidatesOutOsm = String.format("./data/bus_point_candidate/bus_candidate_R%s_%s_%s_%s.OSM", route, i, pointGap, bias);
		String pointsOutOsm = String.format("./data/bus_point_candidate/bus_point_R%s_%s_%s_%s.OSM", route, i, pointGap, bias);
	...
	public static void extractSubroutes(String[] routes) throws InterruptedException, IOException {
		...
		for(String r: routes) routeSet.add(r.split("_")[0]);
		StringBuilder sb = new StringBuilder();
		for(String r: routeSet) sb.append(r + " ");
